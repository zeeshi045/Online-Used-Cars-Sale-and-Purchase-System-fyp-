...
.pagination a:hover:not(.active) {background-color:#ddd;}
    ...
    $sql="select  * from `car_list` where model_id=$id LIMIT {$offset},{$limit} ";
    $noResult = true;
           ...
           <img src="Seller/'.$image.' " alt="" width="249px" height="270px">
            ...
            if($noResult) {
                echo '<div class="jumbotron jumbotron-fluid">
                    <div class="container">
                        <p class="display-4">Sorry In this Model No items available.</p>
...
$sql1="select * from car_list where model_id=$id ";
...
if(mysqli_num_rows($result)>0){
  ...
  if($page>1){
    echo '<a href="modelview.php? page='.((int)$page-1).' & modelid='.$id.'">Prev</a>';
  }
...
echo '<a class="'.$active.'" href="modelview.php?page='.$m.' & modelid='.$id.'">'.$m.'</a>';
  }
  if($total_page>$page){
    echo '<a  href="modelview.php?page='.((int)$page+1).' & modelid='.$id.'" >Next</a>';
  }
