...
if (isset($_POST['submit'])) {
    ...
    $email = mysqli_real_escape_string($mm, $_POST['email']);
    ...
    $user_check_query = "SELECT * FROM seller WHERE  sname='$username' OR email='$email' LIMIT 1";
    $result = mysqli_query($mm, $user_check_query);
    ...
    if (count($errors) == 0) {
        ...
        $query = "INSERT INTO seller (sname,password,email, phone) 
                  VALUES('$username','$password', '$email','$number')";
        mysqli_query($mm, $query); 
...
<body>
    ...
    <!-- Navbar End -->
        ...
        <form method="post" >
            ...
            <button name="submit" class="w-100 bg-primary fs-4 text-white border-none">Register</button>
        </form>
        </div>
